...
flextable(mngmt_table) %>% 
  set_header_labels(Control.type = "Control type") %>% 
  theme_booktabs() %>%
  bold(part = "header") %>% 
  set_caption(caption = "Summary of management measures") %>%
  autofit() %>% 
  bg(bg = "wheat", part = "header") %>% 
  bg(j = 2, bg = cols, part = "body")
```
...
# create a function to output the table by "flextable"
output_as_flextable <- function(data, title){
  flextable(data) %>%
    theme_booktabs() %>%
    bold(part = "header") %>%
...
output_as_flextable(data_table_catch, "Catch data")

output_as_flextable(data_table_bio, "Biological data")
```
