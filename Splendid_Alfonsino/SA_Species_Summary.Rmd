...
CatchDataTable <- data.frame(read.csv("data/SA_Species_Summary_CatchDataTable.csv", header = T))%>%
    kbl(col.names = c("Data type", "Country", "Gear", "Years available", "Comments")) %>%
    kable_paper(full_width = F) %>%
    row_spec(0, bold = T, color = "white") %>%
    column_spec(3, width = "15em") %>%
    kable_styling(font_size = 15, bootstrap_options = c("basic", "striped"))

BioDataTable <- data.frame(read.csv("data/SA_Species_Summary_BioDataTable.csv", header = T)) %>% 
  kbl(col.names = c("Data type", "Country", "Years available", "Comments")) %>%
    kable_paper(full_width = F) %>%
    row_spec(0, bold = T, color = "white") %>%
    column_spec(3, width = "15em") %>%
    kable_styling(font_size = 15, bootstrap_options = c("basic", "striped"))
